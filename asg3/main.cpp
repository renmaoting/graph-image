...
//
// Further tuned by Joshua Levine
// Modified to support OpenImageIO instead of Magick++
// for CPSC 4040/6040, Sep. 4, 2013
//
// MODES:
...
int writeimage(string outfilename){
   ...
   int channels = 4;
   unsigned char oiio_pixels[WinWidth*WinHeight*channels];

   //grab the four channel data from display window
   glReadPixels(0,0,WinWidth,WinHeight,GL_RGBA,GL_UNSIGNED_BYTE,oiio_pixels);
   int scanlinesize = WinWidth * channels * sizeof(oiio_pixels[0]);
   ...
   if(!out->write_image (TypeDesc::UINT8, 
            (unsigned char*)oiio_pixels+(WinHeight-1)*scanlinesize,
            AutoStride, -scanlinesize, AutoStride)) {
      //cerr << "Could not write image: " << outfilename << endl;
      return 0;
   }
...
void CreateMenu()
{
   ...
   glutAddSubMenu ( "Color Mode Options", colormodemenu );
   glutAddSubMenu ( "Shape Mode Options", shapemodemenu );
